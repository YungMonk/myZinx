 ...
 *
 *	用「分层时间轮算法」的方式来管理和维护大量的timer调度，会解决上面的问题。
 */

// TimeWheel 当前调度器的最高级时间轮
type TimeWheel struct {
	...
	// 每个刻度所存放的timer定时器的最大容量（round列表的最大长度）
	maxCap int
...
// 	如果当前的timer的超时时间间隔 小于一个刻度 : 如果没有下一轮时间轮
func (tw *TimeWheel) addTimer(tid uint32, t *Timer, forceNext bool) error {
	...
	// 如果当前的超时时间 大于一个刻度的时间间隔，那么定时器需要放在当前时间轮中
...
// RemoveTimer 删除一个定时器（只能删除当前时间轮中的定时器），根据定时器的id
