...
const (
	...
	BitMicroSeconds
	// 完整文件名称 /home/go/src/zinx/server.go
	BitLongFile
	// 最后文件名   server.go
	BitShortFile
...
// NewZinxLog 创建一个日志：
// out: 标准输出的文件io，prefix: 日志的前缀，flag: 当前日志头部信息的标记位
...
// SetLevel 设置日志级别
func (log *ZinxLogger) SetLevel(level int) {
	log.level = level
}
...
// itoa 将一个整形转换成一个固定长度的字符串，字符串宽度应该是大于0的
// 要确保buffer是有容量空间的
